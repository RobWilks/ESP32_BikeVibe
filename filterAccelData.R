...
#
# read all vibration files, remove frequencies outside the passband, calculate the rms vibrational energy, 
# using a methodology described in the British standard for occupational exposure to vibrating equipment
...
testSpectrum = T # when TRUE used to calibrate power spectrum
MovingAverage = F
smooth = F
plotFFT = F
...
firstFile = 1
lastFile = 302
timeInterval = 1 / fs #time between accelerometer measurements in s
repeatTime = 12 #time between files in s
if (testSpectrum) repeatTime = 1
...
xs_new = c(rep(0, 6))
xs_new[even] = as.integer(round(256 / xs[even]))
xs_new[odd] = as.integer(round(16 * xs[odd] / xs[even]))
test = c(1,2,3,4,5,6)
print(test[odd]/test[even])

dfCal = data.frame(xs, xs_integer, xs_new)                          
...
# for (fileNo in 1:10 * 10)

if (testSpectrum)
{
  fileNoVals = 10^(0:30 * 0.1)
  } else
{
  fileNoVals = 1:100 * 10
  }

for (fileNo in fileNoVals)
# for (fileNo in 1:length(FileNames))
# for (fileNo in firstFile:lastFile)
{
  ...
  if (testSpectrum)
  {
    ...
    # fileNo is used as the simulated frequency
    caly = calx
    calz = calx
